 ...
 */
export async function updateNoteTitle(id: string, title: string): Promise<DbResponse> {
  const db = await getDbConnection();
  ...
  try {
    ...
    db.prepare("UPDATE notes SET title = ?, updated_time = strftime('%s', CURRENT_TIMESTAMP) WHERE id = ?").run(title, id);
